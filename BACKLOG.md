...
| Item | Status | Priority | Effort | Cost | Notes |
|---|---|---|---|---|---|
| Discover feed with sponsored local listings | 🔧 2026-06-04 | P1 | L | €0 build | Core revenue idea — businesses pay a flat monthly fee for context-matched placement (area, weather, group, time). Main driver in the model. **Design revised 2026-06-04**: HERE tile cache killed by HERE ToS (30-day cap + no shared serving); Google seed rejected (only place_id storable). Cold start now **Wikidata** (CC0 — legally cacheable forever, ranked by sitelink notability). 3 layers: community saves, Wikidata notable places, sponsored table. See `docs/MONETIZATION.md`. Organic layers in build; sponsored is Phase 2. |
| Community-layer privacy bundle (ships WITH the discover feed) | 🔧 2026-06-04 | P1 | S–M | €0 | Code shipped with the feed: Settings toggle ("Share my saves anonymously", default on), security-definer aggregate function, min 2-saver threshold. **Dashboard step still needed: run `supabase-migration-v4.sql`.** Privacy policy disclosure remains open (pre-public-launch item). |
